...
void FPtree::InsertTransaction(FP_NODE* &proot, HEADER_TABLE pheader_table, int* ptransaction, int length, int frequency, int nclass_id)
{
	...
	for(i=0;i<length;i++)
	{
		...
		if(pcur_node==NULL || pcur_node->nitem_order>ptransaction[i])
		{
			...
			pnew_node->pclass_distr = NewOneFPCDNode();
			pnew_node->pclass_distr[nclass_id] = frequency;
		...
		else 
		{
			...
			pcur_node->pclass_distr[nclass_id] += frequency;
			pparent = pcur_node;
...
void FPtree::InsertTransaction(FP_NODE* &proot, HEADER_TABLE pheader_table, int* ptransaction, int length, int ntail_pos, int frequency, int *pclass_distr)
{
	FP_NODE *pcur_node, *pnew_node, *pparent, *pleftsib;
	int i, j, t;
	...
	for(i=0;i<length;i++)
	{
		...
		if(pcur_node==NULL || pcur_node->nitem_order>ptransaction[i])
		{
			...
			{
				pnew_node->pclass_distr = NewOneFPCDNode();
				for(t=0;t<gnum_of_classes;t++)
					pnew_node->pclass_distr[t] += pclass_distr[t];
			}
			else 
				pnew_node->pclass_distr = NULL;
			pnew_node->pchild = NULL;
			pnew_node->pparent = pparent;
			pnew_node->prightsibling = pcur_node;
			if(pleftsib!=NULL)
		...
		else 
		{
			...
			{
				for(t=0;t<gnum_of_classes;t++)
					pcur_node->pclass_distr[t] += pclass_distr[t];
			}
			else
				pcur_node->pclass_distr = NULL;
			pparent = pcur_node;
			pcur_node = pcur_node->pchild;
			pleftsib = NULL;
		}
...
void FPtree::CountFreqItems(HEADER_TABLE pheader_table, int nitem_order, int *pitem_classsup_map)
{
	...
	{
		pclass_distr = pitemnode->pclass_distr;
		pfpnode = pitemnode->pparent;
		while(pfpnode!=NULL)
		{
			for(i=0;i<gnum_of_classes;i++)
				pitem_classsup_map[pfpnode->nitem_order*gnum_of_classes+i] += pclass_distr[i];
...
FP_NODE* FPtree::BuildNewFPTree(FP_NODE *pconddb, HEADER_TABLE pnewheader_table, int *pitem_order_map)
{
	...
	while(pitemnode!=NULL)
	{
		...
		if(ntrans_len>1)
		{
			...
			InsertTransaction(pnewroot, pnewheader_table, gptransaction, ntrans_len, ntrans_len, ncount, pitemnode->pclass_distr);
		}
		pitemnode = pitemnode->pnode_link;
	}
...
FP_NODE* FPtree::BuildNewTreeWTail(FP_NODE *pconddb, HEADER_TABLE pnewheader_table, int *pitem_order_map)
{
	...
	FP_NODE *pnewroot, *pitemnode, *pfpnode;
	int ncount, ntrans_len, i, ntail_pos;
	...
	while(pitemnode!=NULL)
	{
		...
		if(ntrans_len>0)
		{
			...
			if(ntrans_len>1)
				InsertTransaction(pnewroot, pnewheader_table, gptransaction, ntrans_len, ntail_pos, ncount, pitemnode->pclass_distr);
		}
		pitemnode = pitemnode->pnode_link;
...
int FPtree::CountTailFullItems(FP_NODE *pconddb, int nfull_sup, int *pfull_items, int num_of_local_items, int ncur_order, bool bnew_tail)
{
	...
	TAIL_NODE *ptail_node=NULL, *pprev_tail_node;
...
int comp_item_freq_des(const void *e1, const void *e2)
{
	...
	else if(p2->nsupport==0)
		return -1;
	else if(p1->nminor_sum<=gndelta)
	{
		if(p2->nminor_sum<=gndelta)
			return 0;
		else 
			return 1;
	}
	else if(p2->nminor_sum<=gndelta)
		return -1;
...
//sort items in descending frequency order
int comp_item_minsum_des(const void *e1, const void *e2)
{
	ITEM_COUNTER *p1, *p2;
	p1 = (ITEM_COUNTER *) e1;
	p2 = (ITEM_COUNTER *) e2;
	...
	if(p1->nsupport==0)
	{
		...
		return -1;
	else if ((p1->nminor_sum>p2->nminor_sum) )
		return -1;
	else if (p1->nminor_sum<p2->nminor_sum)
		return 1;
	else 
...
//sort items in descending frequency order
int comp_hnode_minsum_des(const void *e1, const void *e2)
{
	HEADER_NODE *p1, *p2;
	p1 = (HEADER_NODE *) e1;
	p2 = (HEADER_NODE *) e2;

	if ((p1->nminor_sum>p2->nminor_sum) )
		return -1;
	else if (p1->nminor_sum<p2->nminor_sum)
		return 1;
	else 
