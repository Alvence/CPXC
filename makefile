...
RM 			:= rm
CFLAGS := -g -w 
LFLAGS := -L$(ARFFLIBDIR) -l$(ARFFLIB)  -LGcGrowth -lGcGrowth -lDPM `pkg-config --cflags --libs opencv` -lm -lopencv_core -lopencv_highgui -lopencv_video -lopencv_imgproc 
INCLUDE  := -I$(SRCDIR) -I$(ARFFSRCDIR) -IGcGrowth -IDPM
LD       := g++
